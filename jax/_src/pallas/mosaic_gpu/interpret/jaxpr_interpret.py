...
class JaxprInterpreter:
          ...
          match inner:
            case jax_core.ShapedArray(shape=shape, dtype=dtype):
              if isinstance(dtype, mosaic_gpu_core.BarrierType):
                ...
                memory_space_idx = memory.get_memory_space_idx(
                    memory_space
                )
  ...
  def _interpret_async_store_tmem_p(self, eqn, token: jax.Array, invals):
    ...
    return callback.io_callback(
        ...
        mesh_location=self.mesh_location,
        thread=self.thread,
        dst_allocation_key_as_array=invals[0],
        dst_transforms=jax.tree.unflatten(eqn.params["tree"], invals[2:]),
        vals=invals[1],
    ), []
  ...
  def _interpret_tcgen05_commit_arrive_p(
      self, eqn, token: jax.Array, invals
  ):
    assert eqn.primitive is gpu_primitives.tcgen05_commit_arrive_p
    barrier, *barrier_transforms_flat = invals
