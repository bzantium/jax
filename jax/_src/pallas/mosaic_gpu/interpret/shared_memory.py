...
class PipelineableAsyncTask(Protocol):
  ...
  def __call__(
    ...
    Args:
      pipeline_clock: The clock of the parent task, or None if this task does
        not have a parent or race detection is not enabled.
...
class GPUSharedMemory(
  ...
  next_pipelineable_task_id: int = 0
  ...
  def __init__(
    ...
    self.tcgen05_completion_clocks = collections.defaultdict(
        lambda: self.VectorClock(vector_clock_size)
    )
  ...
  def execute_pipelineable_async_task(
    ...
    with self.lock:
      ...
      if self.detect_races and len(parents) > 0:
        parent_clock = parents[0].result_clock.copy()
...
class Barrier(memory.Allocation):
  ...
  def arrive(
    ...
    with self.cv:
      ...
      # TODO(paulbib): Shouldn't we only notify when the phase completes?
      self.cv.notify_all()
